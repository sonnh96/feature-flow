...
export function labelFeatureType(s: FeatureType): string {
  return { epic: "Epic", feature: "Feature", sub_feature: "Sub-feature" }[s];
}
...
function prefixFromProjectId(projectId: string): string {
  const clean = projectId.replace(/[^a-z0-9]/gi, "").toUpperCase();
  return (clean.slice(0, 4) || "FEAT").padEnd(3, "X");
}
...
function generateFeatureCode(
  ...
  const parent = parentId ? features.find((f) => f.id === parentId) : undefined;
  const prefix = parent?.featureCode || prefixFromProjectId(projectId);
  const width = parent ? 2 : 3;
  const existing = new Set(
    features.filter((f) => f.projectId === projectId).map((f) => f.featureCode),
  );
  let next = 1;
  while (existing.has(`${prefix}-${String(next).padStart(width, "0")}`)) next += 1;
...
function buildSampleFeatures(projectId: string): {
  features: Feature[];
  relations: Relation[];
  history: HistoryEntry[];
} {
  const mk = (p: Partial<Feature> & { name: string; parentId: string | null }): Feature => ({
    id: nanoid(8),
    projectId,
    parentId: p.parentId,
    featureCode: p.featureCode ?? p.code ?? generateFeatureCode(projectId, p.parentId, features),
    code: p.featureCode ?? p.code,
    name: p.name,
    description: p.description ?? "",
    shortDescription: p.shortDescription ?? p.description ?? "",
    longDescription: p.longDescription ?? p.description ?? "",
    featureType: p.featureType ?? deriveFeatureType(p.parentId, features),
...
export const useStore = create<Store>((set, get) => ({
  ...
  createFeature: (data) => {
    const maxOrder = Math.max(
      ...
      ...get()
        .features.filter(
          (f) => f.projectId === data.projectId && f.parentId === (data.parentId ?? null),
        )
        .map((f) => f.order),
    );
    const parentId = data.parentId ?? null;
    const existing = get().features;
    const featureCode =
      data.featureCode ?? data.code ?? generateFeatureCode(data.projectId, parentId, existing);
    const feature: Feature = {
      ...
      featureType: data.featureType ?? deriveFeatureType(parentId, existing),
  ...
  updateFeature: (id, patch, actor = "You") =>
    set((s) => {
      ...
      const updated = {
        ...prev,
        ...patch,
        code: patch.featureCode ?? prev.featureCode,
        currentVersion: patch.currentVersion ?? prev.currentVersion + 1,
        updatedAt: now(),
      };
      ...
      if (patch.businessRules !== undefined && patch.businessRules !== prev.businessRules) {
        entries.push({
          id: nanoid(6),
          featureId: id,
          user: actor,
          userId: actor,
          timestamp: now(),
          action: "Updated business rules",
          eventType: "FeatureFieldChanged",
          fieldName: "businessRules",
          before: prev.businessRules,
          after: patch.businessRules,
          oldValue: prev.businessRules,
          newValue: patch.businessRules,
        });
      }
      return {
  ...
  removeRelation: (id) => set((s) => ({ relations: s.relations.filter((r) => r.id !== id) })),
